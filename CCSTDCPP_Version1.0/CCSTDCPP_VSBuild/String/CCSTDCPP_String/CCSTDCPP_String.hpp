...
#define CCSTDCPP_ISNUL_TERMINATE
...
#ifdef CCSTDCPP_ISNUL_TERMINATE 
	// For default NUL END
#else
#define NUL_CHAR '\0'
#endif
...
#define	DEF_END 0 // using in Building String
	...
	public:
		...
		StringA(const char ch, Size repeatN = 1);
		...
		StringA&		setStr(const char ch, Size repeatN = 1, StringOverLoadSelection thisSelection = def_ConfOverLoadSelection);
		...
		StringA&		append(const char ch, Size repeatN = 1);
		...
		void			insert(const char str, Index toWhere, Size repeatN);
		...
		Position		findSub(const char ch, Index beg = 0, Index end = DEF_END);
		...
		StringA			remove(const char ch, Index beg = 0, Index end = DEF_END);

		// isEqual
		bool			isEqual(const char* str,Index beg = 0, Index end = DEF_END);
		...
		// +
		StringA			operator+(const char* CString);
		StringA			operator+(const char ch);
		StringA			operator+(const StringA& aStr);
...
CCSTDCPP::StringA& CCSTDCPP::StringA::operator=(const char* CString)
{
	...
	Size count = 1;
	for (const char* counter = CString; *counter != '\0'; count++);
...
CCSTDCPP::Position CCSTDCPP::StringA::findSub(const char* CString, Index beg, Index end)
{
...
CCSTDCPP::Position CCSTDCPP::StringA::findSub(const StringA& str, Index beg, Index end)
{
...
CCSTDCPP::Position CCSTDCPP::StringA::findSub(const char ch, Index beg, Index end)
{
	if (ch == '\0')
	{
		return &m_coreString[m_currentSize - 1];
...
CCSTDCPP::PositionGroup CCSTDCPP::StringA::findAllSub(const char ch, Index beg, Index end)
{
	if (end == DEF_END)
	{
		end = m_currentSize;
	...
	PositionGroup ptr2Arr = new Position[DEF_POSGRP_SIZE];
	...
	for (; *fetcher != '\0';fetcher++)
	{
		...
		if(*fetcher != '\0')
		{
			...
			if (countHowMany >= curLimit - 1) {
				...
				PositionGroup newPtr = new Position[curLimit];
...
CCSTDCPP::PositionGroup CCSTDCPP::StringA::findAllSub(const char* CString, Index beg, Index end)
{
	if (end == DEF_END)
	{
		end = m_currentSize;
...
CCSTDCPP::PositionGroup CCSTDCPP::StringA::findAllSub(const StringA& str, Index beg, Index end)
{
	if (end == DEF_END)
	{
		end = m_currentSize;
...
CCSTDCPP::StringA_Array CCSTDCPP::StringA::split(const char* CString)
{
	...
	StringA_Array splitArr = new StringA* [howManySplity + 2]; // 2 specify for 
	// A | B | C
...
CCSTDCPP::StringA CCSTDCPP::StringA::remove(const char* CString, Index beg , Index end)
{
	...
	*copyTo = '\0';
	return StringA(newStringACoreData);
}
